...
- 在 `lib/page/low_admin/ss_node/low_admin_ss_node_page.dart` 新增 VMess/SSR 配置表单，支持
  host/port、alterId、netType、verifyCert、密码、协议、混淆等参数，统一通过生成的 `NodeConfig`/`VmessConfig`/`SsrConfig` 模型提交。
- 表单校验与 UI 依照 vpnType 动态展示配置段，提交前将未使用协议的配置清空，保持后端数据一致。
- 节点卡片新增 VMess/SSR 配置概要，便于管理员确认参数。
